...
分类规则（请严格遵守）：
- habit（习惯）: 需要每天重复的行为，如锻炼、冥想、喝水、阅读30分钟等。关键词：每天、每日、坚持、养成、习惯
- side_quest（支线任务）: 一次性完成的任务，如读完一本书、完成一个项目、学习一个技能、参加一次会议等。关键词：完成、处理、解决、读完、做完
- main_quest（主线任务）: 重要的长期目标或里程碑任务，通常需要数周或数月完成，如掌握一门语言、完成毕业论文、达到健身目标等

判断要点：
1. 如果是"读完某本书"、"完成某个项目"等有明确结束状态的，必须归类为 side_quest
2. 只有每天重复的行为才能归类为 habit
3. 长期大目标才归类为 main_quest
