...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // Extract the operation from the URL path or body
  const pathParts = req.url?.split('/') || [];
  const operation = pathParts[pathParts.length - 1] || req.body.operation;
  ...
  try {
    ...
    switch (operation) {
      ...
      case 'parse-input':
        return handleParseInput(req, res, decoded);
      default:
...
// Chat handler
async function handleChat(req: VercelRequest, res: VercelResponse, decoded: any) {
...
// Suggestions handler
async function handleSuggestions(req: VercelRequest, res: VercelResponse, decoded: any) {
...
// Parse Input handler
async function handleParseInput(req: VercelRequest, res: VercelResponse, decoded: any) {
